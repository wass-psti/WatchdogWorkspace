  ...
  while (Date.now() < deadline) {
    try {
      last = await page.evaluate(async () => {
        ...
        const backendPreflight = await runtime?.get?.('backend-preflight.current') ?? null;
        const backendSettled = backendPreflight?.state === 'ready'
          && backendPreflight?.modules?.boards?.ready === true;
        return {
