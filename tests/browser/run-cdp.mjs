...
const testProgram = String.raw`(async () => {
  ...
  await run('Board explicit rename, configurable Status and single-overlay interaction', async()=>{
    ...
    const titleSaves=[],cellSaves=[],statusSaves=[];let failNextRename=false;
    const coordinator=createBoardOverlayCoordinator();
    const controller=createBoardInlineEditController({state,commands:{setItemTitle:async(command)=>{titleSaves.push(command);if(failNextRename){failNextRename=false;throw new Error('rename persistence failed');}},setCell:async(command)=>cellSaves.push(command),setStatusLabels:async(command)=>statusSaves.push(command),savePreferences:async(_id,prefs)=>prefs,updateColumn:async()=>{},renameGroup:async()=>{}},toast:()=>{},canEdit:()=>true,allColumns:()=>[statusColumn],getCellValue:(item,column)=>column.system_key==='status'?item.status:item.title,optionList:()=>[],renderBoardData:()=>{const cell=root.querySelector('.board-item-name-cell');if(cell)cell.innerHTML='<button id="renameProbe" type="button" data-edit-item-title="item-1">'+state.board.items[0].title+'</button>';},history:null,escapeHtml:String,overlayCoordinator:coordinator,statusLabelsFor:normalizeStatusLabels});
    ...
    assert(state.board.items[0].title==='Original item'&&titleSaves.length===0&&root.querySelector('.board-inline-input'),'rename blur does not save or implicitly close the explicit editor');
    ...
    assert(state.board.items[0].title==='Original item'&&titleSaves.length===0,'explicit X/cancel control restores the prior item name without persistence');
    ...
    assert(state.board.items[0].title==='Confirmed name'&&titleSaves.length===1&&titleSaves.at(-1)?.value==='Confirmed name'&&titleSaves.at(-1)?.expectedValue==='Original item','Enter confirms rename once through the persistence service');
    ...
    assert(state.board.items[0].title==='Confirmed by check'&&titleSaves.at(-1)?.value==='Confirmed by check'&&titleSaves.at(-1)?.expectedValue==='Confirmed name','explicit check/confirm control persists the edited item name');
