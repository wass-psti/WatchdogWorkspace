...
if(bootstrap.status!==0)throw new Error(`M47 browser toolchain bootstrap failed with exit code ${bootstrap.status??'unknown'}.`);
const browser=await findBrowserBinary();
const viteBin=path.join(root,'node_modules','vite','bin','vite.js');
const playwright=path.join(root,'node_modules','.bin','playwright');
if(!fs.existsSync(viteBin)||!fs.existsSync(playwright))throw new Error('M47 browser verification requires npm ci plus the exact modern test toolchain.');
...
try{
  ...
  if(!ready)throw new Error(`M47 Vite server did not become ready.\n${output}`);
  const result=spawnSync(playwright,['test','--config','playwright.config.mjs','tests/modern/e2e/boards-table-group-item-recovery.spec.mjs'],{cwd:root,stdio:'inherit',env:{...process.env,WM_PLAYWRIGHT_BASE_URL:baseURL,WM_PLAYWRIGHT_EXECUTABLE_PATH:browser}});
  if(result.error)throw result.error;
  if(result.status!==0)throw new Error(`M47 browser verification failed with exit code ${result.status??'unknown'}.`);
}finally{
  if(vite.exitCode===null&&vite.signalCode===null)vite.kill('SIGTERM');
  await delay(100);
}
console.log('Stage G M47 Boards Table / Group / Item Recovery browser verification: PASS (scenarios=4; crud=true; visibleSelection=true; preferencePersistence=true; virtualizedKeyboard=true)');
