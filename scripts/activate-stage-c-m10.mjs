...
const targetFile = path.join(root, 'config/stage-c-m10-react-shell-target.ts');
const m9File = path.join(root, 'config/stage-b-m9-client-state-target.ts');
...
if (process.version !== 'v22.16.0') throw new Error(`M10 certification requires Node v22.16.0; current runtime is ${process.version}.`);
if (readState(m9File) !== 'active-certified') {
  setState('blocked-pending-m9-certification');
  throw new Error('M10 requires Stage B M9 to be active-certified before React shell promotion.');
}
if (readState(targetFile) === 'blocked-pending-m9-certification') setState('implementation-complete-pending-certification');
...
for (const [script, label] of [
  ...
  ['typecheck','TypeScript verification against M10 React shell authority'],
]) run(script, label);

setState('active-pending-release-certification');
run('react-shell:check', 'M10 active React shell authority gate');
run('typecheck', 'TypeScript revalidation with active M10 React shell authority');

if (release) {
  run('audit:ci', 'High-severity dependency audit');
  run('release:check', 'Complete production release gate');
  setState('active-certified');
  run('react-shell:check', 'Final M10 certified-state gate');
}
console.log(`\nStage C M10 activation workflow complete for this run. Current state: ${readState(targetFile)}`);
if (!release) console.log('Run `npm run react-shell:activate:release` to complete production release certification.');
