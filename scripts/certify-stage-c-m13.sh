...
if [[ "${1:-}" == "--toolchain-check" ]]; then
  exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
    set -euo pipefail
    printf "M13 governed certification toolchain: %s / npm %s\n" "$(node -v)" "$(npm -v)"
  '
fi
...
exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
  ...
  echo "=== 3. M10 / M11 / M12 / M13 ARCHITECTURE PREFLIGHT ==="
  ...
  echo "=== 5. ACCOUNT / SETTINGS / USER MANAGEMENT REGRESSION CHAIN ==="
  node --experimental-strip-types --disable-warning=ExperimentalWarning verify-account-architecture.mjs
  node --experimental-strip-types --disable-warning=ExperimentalWarning verify-settings.mjs
  node --experimental-strip-types --disable-warning=ExperimentalWarning verify-rbac-user-management.mjs
  npm run verify:ui

  echo "=== 6. DEV BROWSER MANAGEMENT OWNERSHIP CONTRACT ==="
  npm run verify:dev
