...
function prepare() {
  ...
  for (const [from, to] of [
    ['scripts/finalize-stage-g-m46.sh', 'scripts/finalize-stage-g-m46.sh'],
    ['scripts/lib/stage-g-m46-certification-tree.mjs', 'scripts/lib/stage-g-m46-certification-tree.mjs'],
    ['config/stage-g-m46-boards-backend-data-contract-recovery-target.ts', 'config/stage-g-m46-boards-backend-data-contract-recovery-target.ts'],
    ['config/stage-g-m46-board-backend-contract.ts', 'config/stage-g-m46-board-backend-contract.ts'],
  ...
  chmodSync(join(project, 'src', 'm46-executable-fixture.sh'), 0o755);
  try { fsSymlink('m46-certification-source.txt', join(project, 'src', 'm46-source-link')); } catch {}
...
function commitFixture(ctx) {
  ...
  for (const args of [
    ['init', '-q'],
    ['config', 'user.name', 'M46 Finalizer Fixture'],
    ['config', 'user.email', 'm46-fixture@example.invalid'],
    ['add', '-A'],
    ['commit', '-qm', 'fixture'],
  ]) {
...
function run(ctx, env = {}) {
  return spawnSync('bash', ['scripts/finalize-stage-g-m46.sh'], {
    cwd: ctx.project,
    encoding: 'utf8',
    env: { ...process.env, PATH: `${ctx.bin}:${process.env.PATH || ''}`, M46_SOURCE_COMMIT: ctx.commit || '0123456789abcdef0123456789abcdef01234567', ...env },
  });
}

function assertNoNewPass(ctx) {
  const out = join(ctx.project, 'm46-certified-artifacts-upload');
  assert.equal(existsSync(join(out, `${base}.zip`)), false, 'failed M46 finalization must not publish a certified ZIP');
  assert.equal(existsSync(join(out, `${base}-PASS.txt`)), false, 'failed M46 finalization must not publish a PASS record');
}
...
// Case 1: invalid source binding fails before any certification work.
{
  ...
  try {
    ...
    commitFixture(ctx);
    const result = run(ctx, { M46_SOURCE_COMMIT: 'not-a-commit' });
    assert.notEqual(result.status, 0, 'invalid commit binding must fail M46 finalization');
    assert.match(`${result.stdout}${result.stderr}`, /invalid M46 source commit binding/i);
    assertNoNewPass(ctx);
...
// Case 2: any required pre-gate failure aborts without replacing prior artifacts.
{
  const ctx = prepare();
  try {
    installNpmStub(ctx, `case "$*" in\n  *board-backend-contract:browser*) echo simulated-browser-failure >&2; exit 41 ;;\n  *) exit 0 ;;\nesac`);
    const out = join(ctx.project, 'm46-certified-artifacts-upload');
    ...
    assert.notEqual(result.status, 0, 'required browser gate failure must fail M46 finalization');
...
// Case 3: source mutation by a nominally successful gate is detected before staging.
{
  const ctx = prepare();
  try {
    installNpmStub(ctx, `if [ ! -f .m46-mutated ]; then\n  printf '%s\\n' 'mutated-during-gate' >> src/m46-certification-source.txt\n  : > .m46-mutated\nfi\nexit 0`);
    commitFixture(ctx);
    const result = run(ctx);
    assert.notEqual(result.status, 0, 'source mutation during pre-gates must fail M46 finalization');
...
// The run deliberately stops at the later historical gate after both staging parity checks pass.
{
  ...
  try {
    installNpmStub(ctx, 'exit 0');
    writeFileSync(join(ctx.project, 'verify-stage-g-m46-boards-backend-data-contract-recovery.mjs'), 'process.exit(0);\n');
    ...
    assert.match(output, /M46 certification tree parity: PASS/i);
    assert.match(output, /reached-post-state-gate/i);
    assert.doesNotMatch(output, /staged active-certified M46 payload does not match/i);
    assertNoNewPass(ctx);
  } finally { rmSync(ctx.sandbox, { recursive: true, force: true }); }
}
...
// the staged library emit an empty digest even though source/stage parity itself was valid.
{
  ...
  try {
    ...
    const script = join('scripts', 'lib', 'stage-g-m46-certification-tree.mjs');
...
console.log('Stage G M46 finalizer fail-closed verification: PASS (invalid commit binding, required-gate failure, prior-artifact preservation, source-drift rejection, deterministic staging parity, and realpath-stable certification-tree CLI execution)');
