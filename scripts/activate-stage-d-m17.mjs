...
const target = path.join(root, 'config/stage-d-m17-tanstack-table-evaluation-target.ts');
...
if (process.version !== 'v22.16.0') throw new Error(`M17 activation requires Node v22.16.0; current runtime is ${process.version}.`);
if (readState('config/stage-d-m16-board-component-decomposition-target.ts') !== 'active-certified') throw new Error('M16 must remain active-certified before M17 activation.');
for (const [script, label] of [
  ...
  ['tanstack-table-evaluation:check','M17 TanStack Table evaluation gate'],
  ['lint:eslint','Governed ESLint gate'],
  ['typecheck','TypeScript verification against M17 evaluation boundaries'],
]) run(script, label);
setState('active-pending-release-certification');
run('tanstack-table-evaluation:check', 'M17 active evaluation authority gate');
run('typecheck', 'TypeScript revalidation with active M17 evaluation architecture');
if (release) {
  run('audit:ci', 'High-severity dependency audit');
  run('release:check', 'Complete production release gate');
  setState('active-certified');
  run('tanstack-table-evaluation:check', 'Final M17 certified-state gate');
}
console.log(`\nStage D M17 activation workflow complete for this run. Current state: ${readState('config/stage-d-m17-tanstack-table-evaluation-target.ts')}`);
