...
if [[ "${1:-}" == "--toolchain-check" ]]; then
  exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
    set -euo pipefail
    printf "M12 governed certification toolchain: %s / npm %s\n" "$(node -v)" "$(npm -v)"
  '
fi
...
exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
  ...
  echo "=== 3. M10 / M11 / M12 ARCHITECTURE PREFLIGHT ==="
  ...
  echo "=== 5. AUTHENTICATION / UI HISTORICAL CHAIN ==="
  node verify-auth-backend.mjs
  node verify-auth-signin.mjs
  node verify-v1174-auth-verification.mjs
  node verify-rbac-user-management.mjs
  npm run verify:ui

  echo "=== 6. DEV BROWSER AUTHENTICATION OWNERSHIP CONTRACT ==="
  npm run verify:dev
