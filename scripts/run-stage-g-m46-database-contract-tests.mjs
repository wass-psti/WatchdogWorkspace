...
const LOCAL_PROJECT_ID='work-management-m46-contract-tests';
...
const run=(command,args,label,options={})=>{
  ...
  if(result.error){if(result.error.code==='ENOENT')throw new Error(`${command} is required for M46 local database contract verification.`);throw result.error;}
...
try{
  ...
  console.log('\n================ Bootstrap authoritative M46 schema snapshot ================');
  ...
  if(schema.status!==0)throw new Error(`M46 authoritative schema bootstrap failed with exit code ${schema.status??'unknown'}.`);
  supabase(workdir,['test','db','--local',testsPath],'Run M46 pgTAP Board backend/data-contract suite');
  console.log('\nStage G M46 local Database/RLS contract verification: PASS (schemaSnapshot=true; pgTAP=14; rpcs=40; tables=9)');
}finally{cleanup();}
