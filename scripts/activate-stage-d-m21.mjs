...
const target = path.join(root, 'config/stage-d-m21-rich-item-workspace-target.ts');
...
if (process.version !== 'v22.16.0') throw new Error(`M21 activation requires Node v22.16.0; current runtime is ${process.version}.`);
if (readState('config/stage-d-m20-board-collaborative-realtime-target.ts') !== 'active-certified') throw new Error('M20 must remain active-certified before M21 activation.');
for (const [script, label] of [
  ['governance:restore','Synchronize repository governance artifacts'], ['dependencies:ensure','Ensure exact lockfile dependencies'], ['governance:check','Package governance gate'], ['security:check','Security baseline gate'], ['react:check','React composition boundary gate'], ['design-system:check','M4 design-system gate'], ['interactions:check','M5 interaction gate'], ['runtime-schemas:check','M6 runtime-schema gate'], ['supabase-client:check','M7 Supabase adapter gate'], ['tanstack-query:check','M8 TanStack Query gate'], ['client-state:check','M9 client-state gate'], ['react-shell:check','M10 React shell gate'], ['global-overlays:check','M11 global overlay gate'], ['authentication-ui:check','M12 authentication UI gate'], ['account-settings-users:check','M13 management UI gate'], ['shared-app-ui:check','M14 shared application UI gate'], ['board-presentation:check','M15 React Board presentation facade gate'], ['board-components:check','M16 Board component decomposition gate'], ['tanstack-table-evaluation:check','M17 TanStack Table evaluation gate'], ['board-virtualization:check','M18 Board virtualization gate'], ['drag-drop-evaluation:check','M19 Drag-and-drop evaluation gate'], ['board-realtime:check','M20 Board collaborative Realtime gate'], ['rich-item-workspace:check','M21 Rich Item Workspace gate'], ['lint:eslint','Governed ESLint gate'], ['typecheck','TypeScript verification against M21 Rich Item Workspace boundaries'],
]) run(script, label);
setState('active-pending-release-certification');
run('rich-item-workspace:check', 'M21 active Rich Item Workspace authority gate');
run('typecheck', 'TypeScript revalidation with active M21 boundary');
if (release) { run('audit:ci','High-severity dependency audit'); run('release:check','Complete production release gate'); setState('active-certified'); run('rich-item-workspace:check','Final M21 certified-state gate'); }
console.log(`\nStage D M21 activation workflow complete for this run. Current state: ${readState('config/stage-d-m21-rich-item-workspace-target.ts')}`);
