...
import { computeM44CertificationTreeDigest } from './lib/stage-g-m44-certification-tree.mjs';

const root=path.resolve(import.meta.dirname,'..');
const dir=path.join(root,'m44-certified-artifacts-upload');
const base='Work-Management-App-v1.43.2-Stage-G-M44-Certified-Baseline';
const zip=path.join(dir,`${base}.zip`);
const pass=path.join(dir,`${base}-PASS.txt`);
const fail=(message)=>{throw new Error(`M44 certified artifact verification failed: ${message}`);};
...
const expectedCommit=process.env.M44_EXPECTED_SOURCE_COMMIT||process.env.GITHUB_SHA||'';
if(expectedCommit&&sourceCommit!==expectedCommit)fail(`source commit mismatch: expected ${expectedCommit}, received ${sourceCommit}`);
const liveDigest=computeM44CertificationTreeDigest(root).digest;
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'m44-certified-'));
try{
  ...
  const target=fs.readFileSync(path.join(project,'config/stage-g-m44-management-authority-consolidation-target.ts'),'utf8');
  const status=fs.readFileSync(path.join(project,'RELEASE-STATUS-v1.43.2-STAGE-G-M44-MANAGEMENT-AUTHORITY-CONSOLIDATION.md'),'utf8');
  ...
  const packagedDigest=computeM44CertificationTreeDigest(project).digest;
...
console.log(`Stage G M44 certified artifact verification: PASS (zipSha=${zipSha}; source=${sourceSha}; commit=${sourceCommit})`);
