...
if [[ "${1:-}" == "--toolchain-check" ]]; then
  exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc 'set -euo pipefail; printf "M20 governed certification toolchain: %s / npm %s\n" "$(node -v)" "$(npm -v)"'
fi
exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
  ...
  echo "=== 3. M20 REALTIME PREFLIGHT ==="; npm run drag-drop-evaluation:check; npm run board-realtime:check
  echo "=== 4. GOVERNED ESLINT FAIL-FAST ==="; npm run lint:eslint
  echo "=== 5. STRICT TYPES ==="; npm run typecheck
  echo "=== 6. REALTIME REGRESSION CHAIN ==="; node --experimental-strip-types --disable-warning=ExperimentalWarning scripts/verify-board-collaborative-realtime-execution.mjs; npm run verify:ui
  ...
  echo "=== 11. FINAL CERTIFICATION STATES ==="; npm run drag-drop-evaluation:status; npm run board-realtime:status
' -- "$ROOT"
