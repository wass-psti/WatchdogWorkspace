...
if [[ "${1:-}" == "--toolchain-check" ]]; then
  exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
    set -euo pipefail
    printf "M17 governed certification toolchain: %s / npm %s\n" "$(node -v)" "$(npm -v)"
  '
fi
exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
  set -euo pipefail
  ...
  npm ci
  echo "=== 3. M17 ARCHITECTURE PREFLIGHT ==="
  ...
  echo "=== 5. STRICT TYPES ==="
  npm run typecheck
  echo "=== 6. TANSTACK TABLE EVALUATION REGRESSION CHAIN ==="
  node --experimental-strip-types --disable-warning=ExperimentalWarning scripts/verify-tanstack-table-evaluation-execution.mjs
  npm run verify:ui
  echo "=== 7. DEV BROWSER REGRESSION CONTRACT ==="
  npm run verify:dev
