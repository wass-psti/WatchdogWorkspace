...
if [[ "${1:-}" == "--toolchain-check" ]]; then
  exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
    set -euo pipefail
    printf "M16 governed certification toolchain: %s / npm %s\n" "$(node -v)" "$(npm -v)"
  '
fi
exec bash "$ROOT/scripts/run-governed-toolchain.sh" /bin/bash -lc '
  set -euo pipefail
  ...
  npm ci
  echo "=== 3. M16 ARCHITECTURE PREFLIGHT ==="
  npm run board-presentation:check
  npm run board-components:check
  echo "=== 4. GOVERNED ESLINT FAIL-FAST ==="
  npm run lint:eslint
  echo "=== 5. STRICT TYPES ==="
  npm run typecheck
  echo "=== 6. BOARD COMPONENT DECOMPOSITION REGRESSION CHAIN ==="
  node --experimental-strip-types --disable-warning=ExperimentalWarning scripts/verify-board-component-decomposition-execution.mjs
  npm run verify:ui
  echo "=== 7. DEV BROWSER BOARD COMPONENT OWNERSHIP CONTRACT ==="
  npm run verify:dev
