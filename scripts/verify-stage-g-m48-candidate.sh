...
cd "$ROOT"
[ "$(node --version)" = 'v22.16.0' ] || { echo "FAIL: M48 candidate verification requires Node v22.16.0; current $(node --version)." >&2; exit 1; }
...
npm run verify
npm run build
echo 'Stage G M48 candidate verification: PASS'
