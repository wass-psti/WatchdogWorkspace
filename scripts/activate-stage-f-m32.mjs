...
const file = new URL('../config/stage-f-m32-observability-target.ts', import.meta.url);
...
try {
  ...
  run('node', ['verify-stage-f-m32-observability.mjs']);
  run('node', ['--experimental-strip-types', 'scripts/verify-observability-execution.mjs']);
  if (release) {
  ...
  await writeFile(file, source);
  run('node', ['verify-stage-f-m32-observability.mjs']);
  console.log('Stage F M32 activation: PASS');
} catch (error) {
