...
function collect(root, current = root, entries = []) {
  for (const dirent of fs.readdirSync(current, { withFileTypes:true }).sort((a,b)=>a.name.localeCompare(b.name))) {
    ...
    if (!stat.isFile()) throw new Error(`Unsupported filesystem entry in M49 certification tree: ${relative}`);
...
export function computeM49CertificationTreeDigest(projectRoot, { includeManifest=false } = {}) {
...
export function compareM49CertificationTrees(leftRoot, rightRoot) {
  const left = computeM49CertificationTreeDigest(leftRoot, { includeManifest:true });
  const right = computeM49CertificationTreeDigest(rightRoot, { includeManifest:true });
...
if (invokedAsScript) {
  ...
  if (compareIndex >= 0) {
    ...
    const comparison = compareM49CertificationTrees(root, path.resolve(other));
    if (process.argv.includes('--json') || process.argv.includes('--manifest-json')) console.log(JSON.stringify(comparison));
    else if (comparison.equal) console.log(`M49 certification tree parity: PASS (digest=${comparison.left.digest}; files=${comparison.left.files})`);
    else {
      console.error(`M49 certification tree parity: FAIL (left=${comparison.left.digest}; right=${comparison.right.digest})`);
    ...
    const result = computeM49CertificationTreeDigest(root, { includeManifest:process.argv.includes('--manifest-json') });
