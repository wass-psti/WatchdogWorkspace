...
function prepareWorkspace(root) {
  ...
  for (const name of Object.keys(MODERN_TEST_TOOLCHAIN)) fs.rmSync(packageBridge(projectRoot, name), { recursive: true, force: true });
  for (const binary of ['vitest', 'playwright']) fs.rmSync(path.join(projectRoot, 'node_modules', '.bin', binary), { force: true });
  return workspace;
}
...
export function materializeModernTestToolchain(root, { offline = false, stdio = 'inherit', timeout } = {}) {
  const projectRoot = path.resolve(root);
  const workspace = prepareWorkspace(projectRoot);
  ...
  try {
    ...
    return { ok: false, status: 1, error, output: error instanceof Error ? error.message : String(error), workspace };
  }
