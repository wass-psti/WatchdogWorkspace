...
const excludedDirectoryNames = new Set([
  ...
  'm47-certified-artifacts-upload',
]);
...
function collect(root, current = root, entries = []) {
  for (const dirent of fs.readdirSync(current, { withFileTypes:true }).sort((a,b)=>a.name.localeCompare(b.name))) {
    ...
    if (!stat.isFile()) throw new Error(`Unsupported filesystem entry in M47 certification tree: ${relative}`);
...
export function computeM47CertificationTreeDigest(projectRoot, { includeManifest=false } = {}) {
...
export function compareM47CertificationTrees(leftRoot, rightRoot) {
  const left = computeM47CertificationTreeDigest(leftRoot, { includeManifest:true });
  const right = computeM47CertificationTreeDigest(rightRoot, { includeManifest:true });
...
if (invokedAsScript) {
  ...
  if (compareIndex >= 0) {
    ...
    const comparison = compareM47CertificationTrees(root, path.resolve(other));
    if (process.argv.includes('--json') || process.argv.includes('--manifest-json')) console.log(JSON.stringify(comparison));
    else if (comparison.equal) console.log(`M47 certification tree parity: PASS (digest=${comparison.left.digest}; files=${comparison.left.files})`);
    else {
      console.error(`M47 certification tree parity: FAIL (left=${comparison.left.digest}; right=${comparison.right.digest})`);
    ...
    const result = computeM47CertificationTreeDigest(root, { includeManifest:process.argv.includes('--manifest-json') });
