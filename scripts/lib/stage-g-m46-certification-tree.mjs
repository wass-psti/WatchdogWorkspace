...
const excludedDirectoryNames = new Set([
  ...
  'm46-certified-artifacts-upload',
]);
const excludedRelativeDirectories = new Set(['supabase/.temp']);
const excludedRelativeFiles = new Set([
  'CHECKSUMS.sha256',
  'config/stage-g-m46-boards-backend-data-contract-recovery-target.ts',
  'RELEASE-STATUS-v1.43.2-STAGE-G-M46-BOARDS-BACKEND-DATA-CONTRACT-RECOVERY.md',
]);
...
function collect(root, current = root, entries = []) {
  for (const dirent of fs.readdirSync(current, { withFileTypes:true }).sort((a,b)=>a.name.localeCompare(b.name))) {
    ...
    if (!stat.isFile()) throw new Error(`Unsupported filesystem entry in M46 certification tree: ${relative}`);
...
export function computeM46CertificationTreeDigest(projectRoot, { includeManifest=false } = {}) {
...
export function compareM46CertificationTrees(leftRoot, rightRoot) {
  const left = computeM46CertificationTreeDigest(leftRoot, { includeManifest:true });
  const right = computeM46CertificationTreeDigest(rightRoot, { includeManifest:true });
...
if (invokedAsScript) {
  ...
  if (compareIndex >= 0) {
    ...
    const comparison = compareM46CertificationTrees(root, path.resolve(other));
    if (process.argv.includes('--json') || process.argv.includes('--manifest-json')) console.log(JSON.stringify(comparison));
    else if (comparison.equal) console.log(`M46 certification tree parity: PASS (digest=${comparison.left.digest}; files=${comparison.left.files})`);
    else {
      console.error(`M46 certification tree parity: FAIL (left=${comparison.left.digest}; right=${comparison.right.digest})`);
    ...
    const result = computeM46CertificationTreeDigest(root, { includeManifest:process.argv.includes('--manifest-json') });
