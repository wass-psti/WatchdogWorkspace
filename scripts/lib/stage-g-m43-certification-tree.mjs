...
const excludedDirectoryNames = new Set([
  ...
  'm43-certified-artifacts-upload',
]);
...
const excludedRelativeFiles = new Set([
  ...
  'RELEASE-STATUS-v1.43.2-STAGE-G-M43-SETTINGS-FUNCTIONAL-RECOVERY.md',
]);
...
function collect(root, current = root, entries = []) {
  for (const dirent of fs.readdirSync(current, { withFileTypes:true }).sort((a,b)=>a.name.localeCompare(b.name))) {
    ...
    if (!stat.isFile()) throw new Error(`Unsupported filesystem entry in M43 certification tree: ${relative}`);
    entries.push({ relative, type:'file', mode, payload:fs.readFileSync(absolute) });
  }
  return entries;
}

export function computeM43CertificationTreeDigest(projectRoot, { includeManifest=false } = {}) {
...
if (invokedAsScript) {
  ...
  const result = computeM43CertificationTreeDigest(root, { includeManifest:process.argv.includes('--manifest-json') });
