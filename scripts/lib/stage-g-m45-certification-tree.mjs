...
const excludedDirectoryNames = new Set([
  ...
  'm45-certified-artifacts-upload',
]);
const excludedRelativeDirectories = new Set(['supabase/.temp']);
const excludedRelativeFiles = new Set([
  'CHECKSUMS.sha256',
  'config/stage-g-m45-boards-collection-route-recovery-target.ts',
  'RELEASE-STATUS-v1.43.2-STAGE-G-M45-BOARDS-COLLECTION-ROUTE-RECOVERY.md',
]);
...
function collect(root, current = root, entries = []) {
  for (const dirent of fs.readdirSync(current, { withFileTypes:true }).sort((a,b)=>a.name.localeCompare(b.name))) {
    ...
    if (!stat.isFile()) throw new Error(`Unsupported filesystem entry in M45 certification tree: ${relative}`);
...
export function computeM45CertificationTreeDigest(projectRoot, { includeManifest=false } = {}) {
...
export function compareM45CertificationTrees(leftRoot, rightRoot) {
  const left = computeM45CertificationTreeDigest(leftRoot, { includeManifest:true });
  const right = computeM45CertificationTreeDigest(rightRoot, { includeManifest:true });
...
if (invokedAsScript) {
  ...
  if (compareIndex >= 0) {
    ...
    const comparison = compareM45CertificationTrees(root, path.resolve(other));
    if (process.argv.includes('--json') || process.argv.includes('--manifest-json')) console.log(JSON.stringify(comparison));
    else if (comparison.equal) console.log(`M45 certification tree parity: PASS (digest=${comparison.left.digest}; files=${comparison.left.files})`);
    else {
      console.error(`M45 certification tree parity: FAIL (left=${comparison.left.digest}; right=${comparison.right.digest})`);
    ...
    const result = computeM45CertificationTreeDigest(root, { includeManifest:process.argv.includes('--manifest-json') });
