...
const checks=[
 ...
 ['mobile keyboard focus',spec.includes('@m53-mobile-keyboard')&&spec.includes("page.keyboard.press('Escape')")&&spec.includes('await expect.poll(')&&spec.includes("document.querySelector('#primarySidebar')?.contains(document.activeElement) === true")&&shell.includes("window.requestAnimationFrame(() => shellQuery<HTMLElement>('#primarySidebar [data-nav], #primarySidebar [data-command]')?.focus())")&&shell.includes('shellMobileRestoreFocus.focus()')],
