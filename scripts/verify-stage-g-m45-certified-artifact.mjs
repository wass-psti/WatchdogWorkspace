...
import { computeM45CertificationTreeDigest } from './lib/stage-g-m45-certification-tree.mjs';

const root=path.resolve(import.meta.dirname,'..');
const dir=path.join(root,'m45-certified-artifacts-upload');
...
const fail=(message)=>{throw new Error(`M45 certified artifact verification failed: ${message}`);};
...
const expectedCommit=process.env.M45_EXPECTED_SOURCE_COMMIT||process.env.GITHUB_SHA||'';
if(expectedCommit&&sourceCommit!==expectedCommit)fail(`source commit mismatch: expected ${expectedCommit}, received ${sourceCommit}`);
const liveDigest=computeM45CertificationTreeDigest(root).digest;
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'m45-certified-'));
try{
  ...
  const target=fs.readFileSync(path.join(project,'config/stage-g-m45-boards-collection-route-recovery-target.ts'),'utf8');
  const status=fs.readFileSync(path.join(project,'RELEASE-STATUS-v1.43.2-STAGE-G-M45-BOARDS-COLLECTION-ROUTE-RECOVERY.md'),'utf8');
  ...
  const packagedDigest=computeM45CertificationTreeDigest(project).digest;
...
console.log(`Stage G M45 certified artifact verification: PASS (zipSha=${zipSha}; source=${sourceSha}; commit=${sourceCommit})`);
