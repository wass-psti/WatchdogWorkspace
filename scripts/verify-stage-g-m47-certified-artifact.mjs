...
import { computeM47CertificationTreeDigest } from './lib/stage-g-m47-certification-tree.mjs';

const root=path.resolve(import.meta.dirname,'..');
const dir=path.join(root,'m47-certified-artifacts-upload');
...
const fail=(message)=>{throw new Error(`M47 certified artifact verification failed: ${message}`);};
...
const semanticsVersion=text.match(/M47 SEMANTICS VERSION: ([^\n]+)/)?.[1]?.trim();
...
if(semanticsVersion!=='1.43.2-m47-v1')fail(`M47 semantic-version binding mismatch: ${semanticsVersion||'missing'}`);
const expectedCommit=process.env.M47_EXPECTED_SOURCE_COMMIT||process.env.GITHUB_SHA||'';
if(expectedCommit&&sourceCommit!==expectedCommit)fail(`source commit mismatch: expected ${expectedCommit}, received ${sourceCommit}`);
const liveDigest=computeM47CertificationTreeDigest(root).digest;
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'m47-certified-'));
try{
  ...
  const target=fs.readFileSync(path.join(project,'config/stage-g-m47-boards-table-group-item-recovery-target.ts'),'utf8');
  const status=fs.readFileSync(path.join(project,'RELEASE-STATUS-v1.43.2-STAGE-G-M47-BOARDS-TABLE-GROUP-ITEM-RECOVERY.md'),'utf8');
  ...
  const packagedDigest=computeM47CertificationTreeDigest(project).digest;
...
console.log(`Stage G M47 certified artifact verification: PASS (zipSha=${zipSha}; source=${sourceSha}; commit=${sourceCommit})`);
