...
const testsPath=resolve(root,'supabase/tests/m47');
const keep=process.env.WM_M47_KEEP_LOCAL_STACK==='1';
...
const run=(command,args,label,options={})=>{
  ...
  if(result.error){if(result.error.code==='ENOENT')throw new Error(`${command} is required for M47 local database verification.`);throw result.error;}
...
if(globalActual!==CERTIFIED_SUPABASE_CLI){
  ...
  if(resolved.error||resolved.status!==0)throw new Error(`M47 requires Supabase CLI ${CERTIFIED_SUPABASE_CLI}. Install it globally or allow npx to resolve ${pinned}.`);
...
if(docker.error||docker.status!==0)throw new Error('A running Docker-compatible container runtime is required for the disposable M47 Supabase test stack.');

const workdir=mkdtempSync(join(tmpdir(),'wm-m47-supabase-'));
...
try{
  ...
  console.log('\n================ Bootstrap authoritative M47 schema snapshot ================');
  ...
  if(schema.error)throw schema.error; if(schema.status!==0)throw new Error(`M47 authoritative schema bootstrap failed with exit code ${schema.status??'unknown'}.`);
  supabase(workdir,['test','db','--local',testsPath],'Run M47 pgTAP table/group/item recovery suite');
  console.log('\nStage G M47 local Database ordering verification: PASS (schemaSnapshot=true; pgTAP=34; m46ContractCompatible=true)');
}finally{cleanup();}
