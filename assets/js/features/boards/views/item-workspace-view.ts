...
function updateStream(data: ItemWorkspaceEnvelope, canComment: boolean, esc: EscapeHtml, formatDate: DateFormatter): string {
...
export function renderItemWorkspace({ state, canEdit, escapeHtml, formatDate, formatDay }: ItemWorkspaceViewOptions): string {
  ...
  const editable = canEdit() && data.permissions.can_edit;
  ...
  else if (tab === 'overview') content = overviewContent(board, item, data, editable, esc, formatDate, formatDay);
  else if (tab === 'updates') content = `<div class="item-updates">${data.permissions.can_comment ? updateComposer(state.itemPanel.updateDraft || '', esc) : '<div class="item-panel-readonly" role="note">View-only access: updates can be read but not posted.</div>'}${updateStream(data, data.permissions.can_comment, esc, formatDate)}</div>`;
  else if (tab === 'files') {
