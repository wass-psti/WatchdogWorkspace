...
check(boardsUi.includes("main.dataset.boardDetailState = 'loading';")&&boardsUi.includes("main.dataset.boardDetailState = 'error';")&&boardsUi.includes("main.dataset.boardDetailState = 'not-found';")&&boardsUi.includes("main.dataset.boardDetailState = 'ready';"),'Board detail publishes explicit loading/error/not-found/ready data-commit state');
check(boardsUi.includes('main.dataset.boardDetailId = String(envelope.board.id);')&&boardsUi.indexOf('main.dataset.boardDetailId = String(envelope.board.id);') > boardsUi.indexOf("patchHost(main.querySelector<HTMLElement>('[data-board-header-host]'), boardHeader());"),'Board detail publishes the loaded Board identifier only after workspace rendering commits');
check(browser.includes("toHaveAttribute('data-board-detail-state', 'ready')")&&browser.includes("toHaveAttribute('data-board-detail-id', boardId)"),'browser suite waits for exact Board detail data-commit readiness and identity');
...
check(browser.indexOf("expect(fixture.calls('wm_duplicate_board')).toHaveLength(1);") < browser.indexOf('const duplicateDetail = await waitForM45BoardDetailReady'),'duplicate RPC cardinality is proven before detail-render assertions');
