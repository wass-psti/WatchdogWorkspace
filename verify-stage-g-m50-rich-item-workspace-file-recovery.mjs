...
ok(['repository-source','m49-certified-artifact','m49-certified-artifact-current-source'].includes(provenanceContext),'M50 provenance context must be repository-source, m49-certified-artifact, or m49-certified-artifact-current-source.');
if(provenanceContext==='repository-source'){
  ...
  if(provenanceContext==='m49-certified-artifact'){
    ok(target.includes("activationState: 'implementation-complete-pending-certification'"),'Historical M49 certified-artifact context must keep M50 pending.');
  }else{
    ok(target.includes("activationState: 'active-certified'"),'Current-source M49 certification context must preserve the already-certified M50 state.');
    const m50Status=read('RELEASE-STATUS-v1.43.2-STAGE-G-M50-RICH-ITEM-WORKSPACE-FILE-RECOVERY.md');
    ok(m50Status.includes('State: active-certified'),'Current-source M49 certification context requires the M50 release status to remain active-certified.');
  }
}
...
ok(m49Finalizer.includes("WM_M50_PROVENANCE_CONTEXT='m49-certified-artifact-current-source' NODE_OPTIONS='--experimental-strip-types --disable-warning=ExperimentalWarning' bash verify-project.sh"),'M49 finalizer must scope the current-source M50 provenance context only to staged active-certified aggregate verification.');
...
for(const token of ["repository-source","m49-certified-artifact","m49-certified-artifact-current-source","unknown-context","active-certified","implementation-complete-pending-certification"]) ok(provenanceContextVerifier.includes(token),`M50 predecessor provenance regression missing ${token}`);
...
const m50Fixture=read('tests/modern/e2e/helpers/m50-rich-item-workspace-fixture.mjs');
ok(m50Fixture.includes("url.pathname === '/rest/v1/rpc/wm_set_board_cell' || url.pathname === '/rest/v1/rpc/wm_set_board_cell_if_current'"),'M50 browser fixture must implement current Board cell mutation RPCs.');
ok(m50Fixture.includes('const requiredSystemColumns = [')&&m50Fixture.includes("system_key:'title'")&&m50Fixture.includes("system_key:'status'")&&m50Fixture.includes("system_key:'assignee'")&&m50Fixture.includes("system_key:'due_date'")&&m50Fixture.includes("system_key:'notes'"),'M50 browser fixture must publish the complete current system-column authority required by Item Workspace core-property saves.');
ok(m50Fixture.includes("board.columns.some((entry) => entry.system_key === definition.system_key)")&&m50Fixture.includes('board.columns.push({'),'M50 browser fixture must backfill only missing system columns without duplicating inherited authoritative columns.');
ok(m50Fixture.includes("columnId == null ? item.title")&&m50Fixture.includes("column?.system_key === 'status' ? item.status")&&m50Fixture.includes("column?.system_key === 'assignee' ? item.assignee_id")&&m50Fixture.includes("column?.system_key === 'due_date' ? item.due_date")&&m50Fixture.includes("column?.system_key === 'notes' ? item.notes"),'M50 fixture must source intrinsic/system-field CAS comparisons from authoritative item state.');
ok(m50Fixture.includes("name === 'wm_set_board_cell_if_current'")&&m50Fixture.includes('WM_BOARD_CELL_CONFLICT'),'M50 fixture must fail stale expected-value CAS writes closed.');
ok(!m50Fixture.includes("url.pathname === '/rest/v1/rpc/wm_update_board_item'"),'M50 browser fixture must not retain the retired whole-item update RPC authority.');
ok(browserSpec.includes("fixture.calls('wm_set_board_cell_if_current')")&&browserSpec.includes("p_expected_value:'Alpha'")&&browserSpec.includes("fixture.calls('wm_update_board_item')).toHaveLength(0)"),'M50 property browser scenario must prove field-scoped title CAS and reject whole-item mutation fallback.');
