# Stage G M47 — Boards Table / Group / Item Recovery — Certification-Ready Checkpoint 28

Status: implementation complete pending certification; the M47 production migration is already applied and semantically attested, while GitHub `main` remains on the certified M46 source commit. Checkpoint 28 is a post-deployment publication-resume checkpoint and forbids migration replay.

Overall M47 completion at this checkpoint: 99% implementation-corrected/certification-ready; the official four-scenario browser gate is now PASS, the 34-assertion M47 pgTAP authority is PASS, retained M44 browser authority is now 2/2 PASS, production migration `20260919165239_stage_g_m47_boards_table_group_item_recovery.sql` is applied and post-deployment release verification is PASS. Checkpoint 27 stopped only at the atomic publication whitespace gate before any commit or push. Checkpoint 28 removes the release-status trailing whitespace, carries the exact applied M47 deployment provenance in the authoritative source package, and converts the production step to attestation-only resume behavior: compatible production skips replay, while any semantic drift fails closed without `db push`, migration-history repair, rollback, or reset. Final PASS remains pending exact publication commit, local exact-commit certification, GitHub push, hosted exact-SHA regressions/certification, hosted artifact verification, and final remote binding.
...
- The next governed transaction must start from certified M46 GitHub source, overlay the complete Checkpoint 28 publication set (including the already-applied M47 provenance), rerun the full candidate gate, re-attest production without mutation, rerun the post-deployment release gate, then complete exact commit/push and hosted certification.
