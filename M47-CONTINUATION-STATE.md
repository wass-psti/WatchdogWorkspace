# Stage G M47 — Boards Table / Group / Item Recovery — Certification-Ready Checkpoint 30

Status: implementation correction complete pending final recertification. GitHub `main` is on corrective M47 commit `522a32e01639fe6a385a9e5ffe84cd3fda0fd996`; local exact-commit M47 certification and the dedicated hosted M47 certification both passed for that SHA, and the hosted certified artifact was uploaded. The aggregate atomic transaction nevertheless stopped fail-closed because the retained M46 hosted workflow re-ran the M45 collection browser suite and exposed a Board-detail readiness race. Checkpoint 30 corrects that runtime/browser synchronization boundary on top of `522a32e…`; production migration replay remains forbidden.

Overall M47 completion at this checkpoint: 99% implementation-corrected/certification-ready. The M47 product/runtime, 34-assertion pgTAP authority, four-scenario M47 Playwright authority, production semantic migration/attestation, global CI, dedicated M45 workflow, and dedicated hosted M47 certification are already green for the published base. Checkpoint 30 adds an explicit Board-detail data-commit readiness contract (`loading` / `error` / `not-found` / `ready`) plus the exact loaded Board ID, and makes retained M45 browser authority wait for that committed identity before asserting detail content. Final PASS for this new corrective source remains pending dependency-backed candidate/release gates, exact corrective publication, hosted M46 regression success, hosted M47 recertification, artifact verification, and final remote binding.

Current dependency-free authority: **PASS — 275 static checks**, **PASS — 50 deterministic checks**, and **PASS — 62 workflow checks**. Retained M45 static authority is **PASS — 138 checks** and now protects the Board-detail readiness contract. No database migration, RPC signature, RLS grant, production data mutation, or M47 semantic migration change is required by this corrective.
