# Stage G M47 — Boards Table / Group / Item Recovery — Certification-Ready Checkpoint 31

Status: implementation correction complete pending final recertification. GitHub `main` is on corrective M47 commit `c2cd7487a3be8ff4139c98f3df556949b177fe78`; Checkpoint 30 passed local exact-commit M47 certification, the dedicated hosted M45 workflow, and the local M47 browser/database authorities, but the aggregate atomic transaction again stopped fail-closed because hosted M46 run `35506818269` re-ran the retained M45 create/open scenario and observed Board-detail `ready` before the expected Board title was present. Checkpoint 31 corrects the application-level commit semantics on top of `c2cd7487…`; production migration replay remains forbidden.

Overall M47 completion at this checkpoint: 99% implementation-corrected/certification-ready. The M47 product/runtime, 34-assertion pgTAP authority, four-scenario M47 Playwright authority, production semantic migration/attestation, and the full local exact-commit certification are already green for the published base. Checkpoint 31 changes Board-detail readiness from a synchronous shell marker into a bounded animation-frame payload-commit contract: `ready` is published only when the current connected React Board presentation host, exact Board ID/name, visible workspace, committed header metadata, and rendered `#board-workspace-title` all agree. Retained M45 browser authority consumes that exact committed name/title boundary. Final PASS for this new corrective source remains pending dependency-backed candidate/release gates, exact corrective publication, hosted M46 regression success, hosted M47 recertification, artifact verification, and final remote binding.

Current dependency-free authority: **PASS — 285 static checks**, **PASS — 50 deterministic checks**, and **PASS — 62 workflow checks**. Retained M45 static authority is **PASS — 144 checks** and now protects the animation-frame Board-payload commit boundary. No database migration, RPC signature, RLS grant, production data mutation, or M47 semantic migration change is required by this corrective.
