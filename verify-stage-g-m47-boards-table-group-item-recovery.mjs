...
has(continuationState, 'Certification-Ready Checkpoint 32', 'M47 continuation handoff identifies the current checkpoint');
...
has(releaseStatus, 'Checkpoint 32 create-board transaction-boundary corrective', 'M47 release status records the hosted create-board payload corrective');
has(continuationState, 'PASS — 296 static checks', 'M47 continuation summary reports the current static authority count');
has(continuationState, 'PASS — 50 checks', 'M47 continuation summary reports the current deterministic authority count');
has(releaseStatus, 'Current M47 static verifier: **PASS (296 checks)**', 'M47 release summary reports the current static authority count');
...
has(boardsUi, "main.dataset.boardDetailState = 'committing';", 'M47 corrective introduces an explicit pre-ready Board-detail commit state');
has(boardsUi, 'function scheduleBoardDetailCommittedReady', 'M47 corrective owns a dedicated Board-detail committed-readiness publisher');
has(boardsUi, 'requestAnimationFrame(() =>', 'M47 corrective validates Board-detail readiness after browser layout/render commit scheduling');
has(boardsUi, "presentationHost?.dataset.wmBoardPresentationRoute === 'workspace'", 'M47 corrective binds readiness to the current React Board presentation route');
has(boardsUi, "String(currentBoard?.name ?? '').trim() === expectedName", 'M47 corrective binds readiness to the authoritative Board payload name');
has(boardsUi, "title?.textContent?.trim() === expectedName", 'M47 corrective proves the rendered Board title before readiness publication');
has(boardsUi, 'main.dataset.boardDetailId = expectedId;', 'M47 corrective publishes the exact loaded Board identifier only after commit validation');
has(boardsUi, 'main.dataset.boardDetailName = expectedName;', 'M47 corrective publishes the committed Board name alongside the identifier');
has(boardsUi, 'BOARD_DETAIL_COMMIT_RETRY_LIMIT = 2', 'M47 corrective bounds source-level recovery from route/render commit races');
...
has(m45Browser, "toHaveAttribute('data-board-detail-name', boardName)", 'retained M45 browser authority waits for the exact committed Board name');
has(m45Browser, "toHaveAttribute('data-board-detail-commit-id', boardId)", 'retained M45 browser authority validates the committed header identity');
has(m45Browser, "locator('#board-workspace-title')).toHaveText(boardName)", 'retained M45 browser authority verifies rendered title content before returning readiness');
...
has(boardsUi, 'data-board-create-name', 'M47 corrective gives the create-board name control a deterministic identity');
has(boardsUi, 'data-board-create-description', 'M47 corrective gives the create-board description control a deterministic identity');
has(boardsUi, 'submittedName !== nameControl.value || submittedDescription !== descriptionControl.value', 'M47 corrective fails closed when the submit snapshot diverges from the live create controls');
has(boardsUi, 'const draft = Object.freeze({', 'M47 corrective freezes the create-board text snapshot before command dispatch');
has(m45Browser, "createDialog.locator('[data-board-create-name]')", 'retained M45 browser authority targets the exact create-board name control');
has(m45Browser, "createDialog.locator('[data-board-create-description]')", 'retained M45 browser authority targets the exact create-board description control');
has(m45Browser, "toHaveValue('M45 Created Board')", 'retained M45 browser authority proves the Board name DOM value before submit');
has(m45Browser, "toHaveValue('Created through M45 collection recovery')", 'retained M45 browser authority proves the description DOM value before submit');
has(m45Browser, "JSON.parse(createCalls[0]?.rawBody || '{}')", 'retained M45 browser authority proves the raw create RPC JSON payload');
